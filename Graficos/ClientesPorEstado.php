...
<html>
    <head>
        ...
        <title>Listar Clientes Por Estado</title>

        <?php
        require_once ".." . DIRECTORY_SEPARATOR . "autoload.php";
        $padrao = new padrao();
        $vetor = $padrao->returnClientePorEstado();
        ...
        <script type="text/javascript">
            ...
            function drawChart() {
                var data = google.visualization.arrayToDataTable([
                    ['Estados', 'Quantidade Clientes'],
<?php
for ($valor = 0; $valor < $ndados; $valor++) {
    $i = 0;
    ?>
                        [<?php $varteste = "'" . $vetor[$valor][$i + 1] . "'";
    echo $varteste ?>, <?php echo ($vetor[$valor][$i]) ?>],
<?php } ?>
                ]);
