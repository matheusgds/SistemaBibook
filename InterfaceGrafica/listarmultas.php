...
<html>
    ...
    <body>
        <div class="container-fluid">
           <nav class="navbar navbar-expand-lg navbar-dark bg-primary">
            ...
            if ($tipo == 1) {
                if ($procurar != "") {
                    $sql = "SELECT * FROM Multa WHERE idMulta = $procurar ORDER BY idMulta";
                } else {
                    $sql = "SELECT * FROM Multa ORDER BY idMulta";
                }
            } else if ($tipo == 2) {
                $sql = "SELECT * FROM Multa WHERE status = $procurar ORDER BY status";
            }

            require_once ".." . DIRECTORY_SEPARATOR . "autoload.php";
            $multa = new Multa();

            $vet = $multa->PesquisarTodos($sql);
            $count = count($vet);
            ?>
            <br><br>
            <h1>Dados:</h1>
            ...
            <table class="table table-striped table-hover" border="1px" bgcolor="#9dff8c">
                <thead>
                    <tr> 
                        ...
                        <th scope="col" bgcolor="#78ad6f">Status</th>
                        <th scope="col" bgcolor="#78ad6f">ID Locacao</th>
                ...
                <tbody>
                    <?php for ($index = 0; $index < $count; $index++) {
                        ...
                        <tr>
                            ...
                            <td class="table-success"><?php echo $vet[$index]->getValor(); ?></td>
                            <td class="table-success"><?php echo $vet[$index]->getStatus(); ?></td>
                            <td class="table-success"><?php echo $vet[$index]->getIdlocacao(); ?></td>
                            <?php $number = $vet[$index]->getId(); ?>
                            <?php $linkimgeditar = ".." . DIRECTORY_SEPARATOR . "IMG" . DIRECTORY_SEPARATOR . "Edit.png"; ?>
                            <td class="table-success"><button  type="button" id="btn1" onclick="EditCity(<?php echo $number ?>)" ><img src=<?php echo $linkimgeditar ?>> </button></td>
                        </tr>
    ...
    <script type='text/javascript'>
        ...
        function EditCity(valor) {
            var link = "editarmulta.php?multa=";
            link = link + valor;
